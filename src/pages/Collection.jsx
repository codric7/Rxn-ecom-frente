...
export default function Collection() {
  ...
  const sortProducts = (e)=>{
     ...
     let fprodCopy = filterProducts.slice();

     switch(sortType){
      case 'low-high':
        setFilterProducts(fprodCopy.sort((a,b)=>(a.price - b.price)));
        break;

      case 'high-low':
        setFilterProducts(fprodCopy.sort((a,b)=>(b.price - a.price)));
        break;
      ...
      {/* FIlter Options */}
        <div className='min-w-60'>
          <p onClick={()=>{setShowFilter(!showFilter)}} className='my-2 text-xl flex items-center cursor-pointer'>FILTERS
            <img  className={`h-3 sm:hidden ml-3 ${showFilter ? 'rotate-90' : ''}`} src={assets.dropdown_icon} alt="" />
          </p>
